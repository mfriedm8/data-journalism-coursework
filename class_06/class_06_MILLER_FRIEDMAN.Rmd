...
install.packages('arcos', repos = "http://cran.us.r-project.org")
...
manufacturers_maryland <- total_manufacturers_state(state="MD", key=key)

```

```{r}

county_pills_maryland <- summarized_county_annual(state="MD", key=key)

```

```{r}

addresses_baltimore_county <- buyer_addresses(county="Baltimore", state="MD", key=key)
...
#What county in Maryland had the most pills sent to it in 2012?
county_pills_maryland %>%
  filter(year==2012)%>%
...
county_pills_maryland <- clean_names(county_pills_maryland)

```

```{r}
#What manufacturer sent the most pills to Baltimore County?

manufacturers_maryland %>%
  filter(buyer_county=="BALTIMORE") %>%
  arrange(desc(total_dosage_unit))
```

```{r}

#What manufacturer sent the most pills to Howard County? 
manufacturers_maryland %>%
  filter(buyer_county=="HOWARD") %>%
  arrange(desc(total_dosage_unit))

```

```{r}
#What pharmacy received the most pills in Maryland in 2008?
buyer_totals %>%
...
#How many pills per person were sent to Maryland in 2009?
#I need a little bit of help with this because my joins don't seem to be working properly for some reason. 
