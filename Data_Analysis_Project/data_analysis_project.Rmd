...
maryland_rural_pills <- summarized_county_annual(key = key) %>%
  clean_names() %>%
   select(buyer_state, buyer_county, year, dosage_unit, countyfips) %>%
  filter(buyer_state == "MD", buyer_county == "GARRETT, ALLEGANY, WASHINGTON, FREDERICK, CARROLL, HARFORD, CECIL, KENT, QUEEN ANNE'S, CAROLINE, TALBOT, DORCHESTER, WICOMICO, WORCESTER, SOMERSET, CHARLES, CALVERT, ST. MARY'S")

#21 How many pills were distributed to majority urban/suburban counties in Maryland?

maryland_urban_pills <- summarized_county_annual(key = key) %>%
  clean_names() %>%
   select(buyer_state, buyer_county, year, dosage_unit, countyfips) %>%
  filter(buyer_state == "MD", buyer_county == "BALTIMORE, BALTIMORE CITY, MONTGOMERY, HOWARD, ANNE ARUNDEL, PRINCE GEORGE'S")
...
wv_urban_pills <- summarized_county_annual(key = key) %>%
  clean_names() %>%
   select(buyer_state, buyer_county, year, dosage_unit, countyfips) %>%
  filter(buyer_state == "WV", buyer_county == "BERKELEY, BROOKE, CABELL, HANCOCK, JEFFERSON, KANAWHA, MARSHALL, MINERAL, OHIO, PUTNAM, WAYNE, WOOD")

#25 How many pills were distributed to rural counties in West Virginia?

wv_rural_pills <- summarized_county_annual(key = key) %>%
  clean_names() %>%
   select(buyer_state, buyer_county, year, dosage_unit, countyfips) %>%
  filter(buyer_state == "WV", buyer_county == "BERKELEY, BROOKE, CABELL, HANCOCK, JEFFERSON, KANAWHA, MARSHALL, MINERAL, OHIO, PUTNAM, WAYNE, WOOD")
