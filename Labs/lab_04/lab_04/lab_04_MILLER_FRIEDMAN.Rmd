...
mingo%>%
  group_by(combined_labeler_name, reporter_name,    buyer_name, dosage_unit)%>%
  summarise(number_of_pills = sum(dosage_unit))%>%
  arrange(desc(number_of_pills))

#Actavis->Mckesson->Strosnider was the most significant combo
```
...
mingo%>%
  group_by(combined_labeler_name, reporter_name, reporter_address1, reporter_city,   buyer_name, dosage_unit)%>%
  summarise(number_of_pills = sum(dosage_unit))%>%
  arrange(desc(number_of_pills))
...
#I could most likely use a rename function and apply the name "Cardinal Health" to any value that contains the word "Cardinal." 
