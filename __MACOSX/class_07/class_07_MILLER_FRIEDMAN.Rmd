...
ggplot(logan_county_pills_per_year) +
  ...
  labs(x="Year", y="Total Pills", title="In Logan County, opioid shipments rose until 2009, decline thereafter", subtitle="Total pills shipped to Logan County by year", caption="Source: DEA ARCOS database, via Washington Post") +
...
west_virginia_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", (buyer_county %in% c("KANAWHA", "CABELL", "MINGO", "RALEIGH", "LOGAN"))) %>%
...
ggplot(west_virginia_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="Opoioid shipments in most major West Virginia counties stay steady or rise, but for Mingo County", subtitle="Total pills per year shipped to Cabell, Kanawha, Logan, Mingo, and Raleigh counties", caption="DEA ARCOS database, via Washington Post", fill="County") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels=comma) +
...
maryland_2012 <- maryland_pills_2012 %>%
  ...
  select(buyer_county, dosage_unit, population, year) %>%
  filter(year=="2012") %>%
...
pills_per_person_by_income <- md_county_median_household_income %>% 
  mutate(countyfips=(GEOID)) %>%
  inner_join(fipsmaryland_2012, by="countyfips") %>%
  select(buyer_county, countyfips, year.x, pills_per_person, estimate)%>%
  filter(year.x=="2012")
...
ggplot(pills_per_person_by_income) +
  geom_point(aes(estimate, pills_per_person)) +
  labs(x="Median Household Income", y="Total pills per person in 2012", title="In Maryland during 2012, pills per person fell as median household income rose", subtitle="Pills per person and median household income, Maryland 2012", caption="DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels=comma) +
...
kent_and_montgomery <- maryland_2012 %>%
  select(buyer_county, pills_per_person)

```

```{r}

ggplot(kent_and_montgomery) +
  geom_bar(stat="identity", aes(buyer_county, pills_per_person), fill="forest green") +
  labs(x="buyer_county", y="Total pills per person", title="In Logan County, opioid shipments rose until 2009, decline thereafter", subtitle="Total pills shipped to Logan County by year", caption="Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks= c(ALLEGANY, HOWARD)) +
  scale_y_continuous(labels=comma)
