...
#I loaded all the .csv files using the read_csv function.
...
#The number of records for buyer_annual_by_year is higher than buyer_totals because the records for the first data set are for every year from 2006 to 2012, and buyer_totals is only one value summed up. 

#The number of records in buyer_addresses is higher than buyer_totals because some buyers have multiple addresses.
...
most_pills <- buyer_addresses %>%
  inner_join(buyer_totals, by="buyer_dea_no")

#The VA Consolidated Mail Outpatient Pharmacy had the most total pills sent to it. The address is VA CMOP North Charleston, SC 29405. 

#My guess as to why this pharmacy had so many pills sent there is that this is a pharmacy for veterans. 
...
#Rakesh Kumar Mathur is the practitioner who had the most pills in 2010. He had 337450 pills compared to the next doctor's 212800. His license is currently on probation. I could check it is the same doctor based on his address. 
...
buyer_totals %>% 
  ...
  mutate(pill_rate = total_pills_by_state/population_average) %>%
  arrange(pill_rate)

#West Virginia had the highest rate of total pills per  person sent to it over the period.
...
#The fact that there is no row returned makes sense because the practitioners are by default part of the counties in which they practice. 
...
#There are many buyers in the buyer_addresses table that are not in the buyer_totals table. This makes sense because the buyer_addresses table shows each practitioner, whereas the buyer_totals table shows only the counties. 
