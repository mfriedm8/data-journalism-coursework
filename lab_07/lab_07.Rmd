...
```{r}
# A vast majority of the entities in the not_in_population table are from provinces of the United States, but there are a few at the bottom of the table that are from contiguous states. This could cause an issue because it affects the national dosage unit and therefore our death rate percentages and things like that won't line up exactly with how many pills were distributed during the timeframe we're analyzing. 
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='pills per person',title="West Virginia received the most pills per person in 2009", subtitle = "Pills per person, 2009", caption = "Source: DEA ARCOS database, via Washington Post") +
  theme(legend.position="right") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increased in most states from 2006 to 2012", subtitle = "Pills per person, 2006 to 2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_maryland <- state_geodata_shifted %>%
